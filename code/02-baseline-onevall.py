...
def run_tuning_fit_predict(X_train, 
    ...
    gs = GridSearchCV(estimator=pipeline, 
                       ...
                       cv=min(5, pd.Series(y_train).value_counts().min()), 
                              ## can't do cv on 5 folds if we have > 5 samples in a class
                       )
...
def run_crosscenter_onevall_eval(df_train,
                                 md_train,
                                 df_test, 
                                 md_test, 
                                 all_tasks,
    ...
    for iii,task in enumerate(md_train.disease_type.unique()):
        relevant_centers = md_train.loc[md_train.disease_type==task].data_submitting_center_label.unique()
        md_train_1=md_train.copy().loc[ md_train.data_submitting_center_label.isin(relevant_centers)] 
        md_test_1 =md_test.loc[md_train_1.index]
        df_train_1=df_train.loc[md_train_1.index]
        df_test_1=df_test.loc[md_train_1.index]
        
        
        if md_train_1.data_submitting_center_label.nunique()>=2:
            ...
            for train_inds_, test_inds_ in logo.split(
                ...
                for val_run in [True]:#[False, True]:
    ...
    #                 if val_run:
    #                     ## make sure there's no site overlap
    #                 train_inds = np.array( [ i for i in train_inds
    #                                         if md_train_1.tissue_source_site_label.values[i] 
    #                                         not in 
    #                                      np.unique( md_test_1.tissue_source_site_label.values[test_inds] ) ] )

    #                 elif len( test_inds.shape )>0:
    #                      ## make sure there's no site overlap
    #                 test_inds = np.array( [ i for i in test_inds 
    #                                        if md_test_1.tissue_source_site_label.values[i] 
    #                                        not in 
    #                                      np.unique( md_train_1.tissue_source_site_label.values[train_inds] ) ] )

                    if len( train_inds.shape )>0 and len( test_inds.shape )>0:
                        if train_inds.shape[0] >= min_n and \
                             test_inds.shape[0] >= min_n:
                            ...
                            y = (md_train_1.disease_type.values == task).astype(float)
                            y_train=y[train_inds]
                            y_test=y[test_inds]

                            ## remove overlap of train/test sites
                            ...
                            if np.unique(y_test).shape[0]>1 and np.unique(y_train).shape[0]>1 and\
                                        pd.Series(y_test).value_counts().min()>=min_n and\
                                        pd.Series(y_train).value_counts().min()>=min_n:
...
def make_one_analysis_run(train_nm='oncog_v1', 
    ...
    res_df = run_crosscenter_onevall_eval(df_train,
                                          md_train,
                                          df_test, 
                                          md_test, 
                                          all_tasks,
...
def main(results_path='../results/02-baseline-onevall.csv', 
         seed=42):
